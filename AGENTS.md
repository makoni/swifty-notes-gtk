...
- Anything Foundation formats rather than gettext — dates, times, numbers — goes through `interfaceLocale()`. It reads the pinned language first (Foundation ignores `LANGUAGE`, so a Russian interface would otherwise print English dates), then what the session asked for via swift-adwaita's `sessionLocaleIdentifier(for: .time)`, then `Locale.current`. **`Locale.current` does not follow the environment on Linux** — measured on Swift 6.3.2 it is `en_001` whatever `LANG`, `LC_ALL` or `LC_TIME` say, while an explicit `Locale(identifier:)` formats correctly — so it cannot stand in for the session's locale, which is what made system-language dates always English (#47).
