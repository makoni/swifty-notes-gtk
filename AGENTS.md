...
- Strings live in `po/<lang>.po`; `po/LINGUAS` is the list of shipped languages and drives both catalogue compilation and metadata rendering. The catalogue guards read `LINGUAS` too, so every shipped language is checked for coverage, orphans, empty translations, fuzzy entries, format specifiers and plural-form count — the last of those against the language's *own* `nplurals`, read from its header rather than tabulated in the test.
...
- Release notes: the `@VERSION@` block — the release being prepared — carries **no** `translate="no"`, so its notes are translated. Every older block carries it. **Freezing a release adds the attribute**: when the next version's block is written, the one it replaces is frozen with its shipped date *and* `translate="no"`, which drops its strings back out of the template. That keeps exactly one release's notes in the catalogue instead of accumulating them — they were 70 of 92 translatable strings before this rule, and they are archival the moment they ship.
- Translatable text in `data/*.metainfo.xml.in` and `data/*.desktop.in` must be **pure ASCII**. gettext 0.23's ITS extractor silently drops non-ASCII from a translatable element — an em dash comes out as nothing, and the msgid reaching the catalogue is missing characters the source still has. `--from-code=UTF-8` does not help, nor does an XML character reference. A guard test enforces it and points at the offending line. Translations themselves are unaffected: they come from `po/` and keep their own typography, so a German entry can hold an em dash the English source cannot. Text under `translate="no"` is exempt and keeps the punctuation it shipped with.
...
- **An interface in English with translated dates means GLib latched the process as untranslated.** GLib decides once per process whether the program is translated at all, and decides "no" when the first `g_dgettext` runs under a locale that neither equals `C` nor starts with `en_` while no catalogue is loaded — `C.UTF-8` is the gap, and it is the default in a container, in the Flatpak sandbox and on the CI runner. GTK makes that first lookup itself during initialization. swift-adwaita's `setLanguage` therefore *exports* the locale it escapes to, so `gtk_init`'s `setlocale(LC_ALL, "")` keeps it; dates keep working either way because Foundation formats them without GLib, which is what makes the symptom recognisable. `A Russian interface reaches the accessibility tree` runs under `LANG=C.UTF-8` on purpose.
- Anything Foundation formats rather than gettext — dates, times, numbers — goes through `interfaceLocale()`, and its order matters: the pinned language, then `sessionLocaleIdentifier(for: .time)`, then the session's `LANGUAGE` (colon-split and normalised), then `Locale.current`. `LANGUAGE` comes **after** the categories: a session that sets `LANGUAGE=en_GB:en` with `LC_TIME=de_DE.UTF-8` — GNOME's "language English (UK), formats Germany" — asked for German dates, and reading `LANGUAGE` first also drops the region (`LANGUAGE=de` beside `LANG=de_AT.UTF-8` prints *Januar* for a session that asked for *Jänner*). It is in the chain at all because the Flatpak sandbox runs `LANG=C.UTF-8`, where the categories say nothing and `LANGUAGE` is the only thing the host session sets. Normalise it rather than passing it through — `LANGUAGE=C` is the idiom for forcing English tool output, and `Locale(identifier: "C")` formats from CLDR's root data. **`Locale.current` does not follow the environment on Linux**: measured on Swift 6.3.2 it is `en_001` whatever `LANG`, `LC_ALL` or `LC_TIME` say, while an explicit `Locale(identifier:)` formats correctly — which is what made system-language dates always English (#47). On Darwin it *is* the user's region, so it comes straight after the pinned language there.
