...
- interface language picker (follow the system, German, English, Spanish, French, Italian, Dutch, Brazilian Portuguese, Russian, Hebrew, Arabic, Japanese, Simplified Chinese, Traditional Chinese, Korean) that applies without a restart, right-to-left languages included
...
- Build an `.AppImage` from that root: `packaging/release/build-appimage.sh --install-root packaging/out/install-root-usr`, then check it with `packaging/release/verify-appimage.sh`. `docs/APPIMAGE.md` covers what the bundle carries, what it borrows from the host, and why spell check needs `enchant-2` installed there — an AppImage is unconfined, but it still cannot override an absolute path compiled into a dependency.
...
The Flatpak manifest template lives in `flatpak/me.spaceinbox.swiftynotes.yml.in` and pins the SwiftPM dependency sources used in CI. GitHub Actions release automation lives in `.github/workflows/release-packages.yml`: it resolves its version from the repository `VERSION` file by default (with an optional `workflow_dispatch` override), fans out into the per-platform builders (deb / rpm / AppImage / flatpak / snap on Linux × {x86_64, arm64}, plus a Developer-ID-signed and notarized DMG for Apple Silicon via the reusable `.github/workflows/release-macos.yml`), and finishes by drafting a GitHub release that bundles every uploaded artifact from the run.
...
Korean, Dutch, Brazilian Portuguese, Russian, Simplified Chinese and Traditional
Chinese, with English carried by the msgids themselves. The app sets the reading
direction from the language it is showing, so Arabic and Hebrew lay out right to
left without a restart.
